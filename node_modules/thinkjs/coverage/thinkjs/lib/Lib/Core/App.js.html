...
<div class="header high">
    ...
    <h2>
        ...
        Statements: <span class="metric">87.5% <small>(154 / 176)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        ...
        Lines: <span class="metric">87.5% <small>(154 / 176)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
...
App.getBaseController = function(http, options, checkAction, ignoreCall){
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!ignoreCall &amp;&amp; isFunction(instance[C('call_method')])) {
...
App.getCallController = function(http){
  ...
  var action = config[2];
  var instance;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (callController &amp;&amp; !APP_DEBUG) {
<span class="cstat-no" title="statement not covered" >    instance = callController;</span>
  }else{
  ...
  if (instance) {
    ...
    http.action = action;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!callController) {
...
App.sendError = function(http, error){
  ...
  http.res.statusCode = C('error_code') || <span class="branch-1 cbranch-no" title="branch not covered" >500;</span>
 ...
 * @return {[type]} [description]
 */
App.run = <span class="fstat-no" title="function not covered" >function(){</span>
  'use strict';
<span class="cstat-no" title="statement not covered" >  if (APP_MODE &amp;&amp; App.mode[APP_MODE]) {</span>
<span class="cstat-no" title="statement not covered" >    return App.mode[APP_MODE]();</span>
  }
<span class="cstat-no" title="statement not covered" >  return App.mode.http();</span>
};
/**
 * 不同模式下的run
 * @type {Object}
...
App.mode = {
  //命令行模式
  cli: <span class="fstat-no" title="function not covered" >function(){</span>
    'use strict';
<span class="cstat-no" title="statement not covered" >    var defaultHttp = thinkHttp.getDefaultHttp(process.argv[2]);</span>
<span class="cstat-no" title="statement not covered" >    thinkHttp(defaultHttp.req, defaultHttp.res).run().then(App.listener);</span>
  },
  //HTTP模式
  http: function(){
    'use strict';
...
App.createServer = function(){
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (C('use_websocket')) {
<span class="cstat-no" title="statement not covered" >    thinkRequire('WebSocket')(server, App).run();</span>
  }
  var host = C('host');
  var port = process.argv[2] || <span class="branch-1 cbranch-no" title="branch not covered" >C('port'); </span>
...
<div class="footer">
    <div class="meta">Generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Tue Nov 11 2014 11:42:26 GMT+0800 (CST)</div>
</div>
